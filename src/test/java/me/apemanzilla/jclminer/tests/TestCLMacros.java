...
/**
 * This class contains multiple JUnit tests that test the OpenCL portions of
 * JCLMiner code. This should be used to ensure that valid results are produced
 * when modifying the OpenCL code.
 * 
 * @author apemanzilla
 *
 */
	...
	public void testMacro_PAD() {
		...
		for (int j = 0; j < inputs.length; j++) {
			...
			CLBuffer<Byte> inputBuf = input.length() > 0 ? context.createByteBuffer(Usage.Input, inputPtr)
					: context.createByteBuffer(Usage.Input, 1), outputBuf = context.createByteBuffer(Usage.Output, 64);
			...
			for (int i = 0; i < 64; i++) {
				if (output[i] != expect[i]) {
					fail(String.format("Mismatch on byte %d when padding item %d: read byte %d, wanted byte %d", i, j,
							output[i], expect[i]));
				}
			}
		}
