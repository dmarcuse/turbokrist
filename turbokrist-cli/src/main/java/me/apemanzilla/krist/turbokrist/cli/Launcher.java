...
public class Launcher {
	...
	public static void listDevices() {
		...
		for (Entry<Integer, CLDevice> e : es) {
			System.out.format("%20s | %11s | %4s\n", e.getValue().getName().trim(), MinerFactory.getSignature(e.getValue()), e.getKey());
		}
	}
	...
	public static void main(String[] args) throws ParseException {
		...
		if (cmd.hasOption('l')) {
			if (verbose) System.out.println("Listing compatible OpenCL devices");
			listDevices();
			System.exit(1);
		} else if (cmd.hasOption('p')) {
			if (verbose) System.out.println("Running system profiler");
			// TODO: run profiler
			System.exit(1);
		} else if (cmd.hasOption('?')) {
			if (verbose) System.out.println("Come on, do you really need verbose output when looking at the help?");
			printHelp();
			System.exit(1);
		}
	}
