...
public class Launcher {
    ...
    static {
        options.addOption(Option.builder("h").longOpt("host").hasArg().argName("address")
                .desc("The Krist address/pool to mine for").build());
        options.addOption(
    ...
    public static void printHelp() {
        ...
        hf.printHelp("turbokrist -h <address>|<host>:<port>:<address>", options);
    }
    ...
    public static void main(String[] args) throws ParseException, MinerInitException {
        ...
        YTCIKristPoolClient pool = null;

        if (host.contains(":")) {
            final String[] parts = host.split(":");

            if (parts.length != 3) {
                System.err.println("Invalid pool format: " + host);
                return;
            }

            try {
                pool = new YTCIKristPoolClient(parts[0], Integer.parseInt(parts[1]), parts[2]);
                pool.connect();

                System.out.println("Connected to pool!");
                System.out.println("Pool MOTD: " + pool.getMOTD());
            } catch (Exception e) {
                e.printStackTrace();
                return;
            }

            options = new MinerOptions(pool.getMiningAddress());
            options.setWorkCoefficient(100);
			options.setStopOnSolution(false);
        } else {
            options = new MinerOptions(host);
        }
        ...
        Controller controller = new Controller(options, pool);
        controller.start();
    }
